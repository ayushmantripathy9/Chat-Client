...
public:
    ...
    void sendFile(std::istream &input, string receiver_id, string filename)
    {
        ...
        ifstream file_size_ptr(filename, ios::binary);
    ...
    void get_file()
    {
        while (true)
        {
            ...
            cout << "Enter id of receiver: ";
            string id;
            getline(cin,id);

            std::ifstream fin("./file_hub/"+fname, std::ifstream::binary);
            sendFile(fin, id, fname);
        }
    }
        ...
        while(true){
            ...
            while (size_transferred < final_size)
            {
                ...
                if(string(recv_buffer)!=SIGNAL_SEND){
                    write_to_file.write( recv_buffer,sizeof(recv_buffer));
...
int main()
{
    ...
    thread recv(&FileClient::receive_file,client);
    client->get_file();
